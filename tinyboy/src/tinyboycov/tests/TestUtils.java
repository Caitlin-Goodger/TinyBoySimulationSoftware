...
public class TestUtils {
   ...
   * @throws Throwable
   */
  public static void checkCoverage(double target, String filename, boolean timeout, boolean gui, int nThreads,
   ...
   * @throws Throwable
   */
  public static void checkCoverage(String name, HexFile firmware, double target, boolean timeout, boolean gui,
      int nThreads, int batchSize) throws Exception {
    if (timeout) {
      String testClassName = TestUtils.class.getName();
      try {
        ProcessTimerMethod.Outcome r = ProcessTimerMethod.exec(TIMEOUT, testClassName, "checkCoverageWithTimeout", name,
            firmware, target, gui, nThreads, batchSize);
        //
  ...
  public static void checkCoverageWithTimeout(String name, HexFile firmware, Double target, Boolean gui,
      Integer nThreads, Integer batchSize) throws Exception {
    ...
    if (coverage.getBranchCoverage() < target) {
      // Indicates a fail
      System.out.println("===============================================");
      System.out.println(name + " (" + String.format("%.2f", coverage.getInstructionCoverage()) + "% instructions, "
          + String.format("%.2f", coverage.getBranchCoverage()) + "% branches, " + time + "ms)");
      System.out.println("===============================================");
  ...
  public static void printDisassembly(HexFile firmware, CoverageAnalysis coverage) {
    ...
    System.out
        .println("Instruction Coverage = " + coveredInstructions + " / " + instructions + " (" + code.size() + ")");
    System.out.println(
        "Branch Coverage = " + coveredBranches + " / " + branches + " (" + coverage.getBranchCoverage() + "%)");
  }
